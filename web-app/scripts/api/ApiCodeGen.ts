...
export default class ApiCodeGen {
    ...
    generate() {
        ...
        let { name, url, pathGlob } = this.apiContext.config
        console.log('start generate ', name, url, pathGlob)
    ...
    protected generateEnum(arr: string[], modelSchma: ApiModelSchma) {
        ...
        for (let propertyName of propNames) {
            ...
            arr.push(`${propertyName}=${property.example},`)
        }
        arr.push('}\n')

        arr.push(`export const ${enumName}Options=[\n`)
        for (let propertyName of propNames) {
            if (propertyName === ENUM_FLAG) {
                continue
            }
            let property = properties[propertyName]
            arr.push(`{label:'${property.description}',value:${property.example}},`)
        }
        arr.push(`]\n`)
        let paramName = toLowerCamelCase(enumName)
        arr.push(
            `export const get${enumName}Text=(${paramName}:${enumName})=>${enumName}Options.find(o=>o.value===${paramName})?.label`
        )
    }
    protected buildFile() {
        ...
        let content = prettier.format(tsContent.join('\n') + this.generateApi(), {
            ...options,
            parser: 'typescript',
        })
        fs.writeFileSync(path.join(apiDir, config.name + `.ts`), content, 'utf8')
    }
